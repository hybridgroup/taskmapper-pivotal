...
describe "TaskMapper::Provider::Pivotal::Comment" do
  ...
  pending "should be able to load all comments based on 'id's" do
  ...
  pending "should be able to load a comment based on id" do
    @comment = @ticket.comment(@comment_id)
    @comment.should be_an_instance_of(@klass)
    @comment.id.should == @comment_id
  end
  
  pending "should be able to load a comment based on attributes" do
    @comment = @ticket.comment(:id => @comment_id)
    @comment.should be_an_instance_of(@klass)
  end
  
  pending "should return the class" do
    @ticket.comment.should == @klass
  end
  
  pending "should be able to create a comment" do # which as mentioned before is technically a ticket update
