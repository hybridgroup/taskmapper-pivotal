...
describe "TaskMapper::Provider::Pivotal::Ticket" do
  before(:all) do
    ...
    ActiveResource::HttpMock.respond_to do |mock|
      ...
      mock.get '/services/v3/projects/93790/activpendingies.xml?occurred_since_date=2010%2F06%2F26', headers, fixture_for('activities'), 200
  ...
  pending "should be able to load all tickets based on an array of ids" do
  ...
  pending "should be able to load all tickets based on attributes" do
  ...
  pending "should be able to load a single ticket" do
  ...
  pending "should be able to create a ticket" do
    @ticket = @project.ticket!(:tpendingle => 'Ticket #12', :description => 'Body')
    @ticket.should be_an_instance_of(@klass)
  end

  pending "should be able to load all tickets based on attributes using updated_at field" do
    @ticket = @project.ticket(@ticket_id)
  ...
  pending "shoule be able to load all tickets based on attributes using created_at field" do
    @ticket = @project.ticket(@ticket_id)
  ...
  pending "should return the requested_by field" do
    @ticket = @project.ticket(@ticket_id)
    @ticket.requestor.should == 'Hong Quach'
  end

  pending "should be able to update a ticket" do
    @ticket = @project.ticket(@ticket_id)
    @ticket.tpendingle = "Hello World"
    @ticket.save.should be_true
  end

  pending "should have all contract fields for tickets" do 
    @ticket = @project.ticket(@ticket_id)
    @ticket.tpendingle.should_not be_nil
    @ticket.description.should_not be_nil
    @ticket.status.should_not be_nil
    @ticket.priorpendingy.should_not be_nil
